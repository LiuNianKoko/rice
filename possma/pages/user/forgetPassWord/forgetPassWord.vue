<template>
	<view class="content">
		...
		<input class="inputBox marignInput" placeholder="请输入新密码(8-16位数字+字母)"  	@input="getNewTel">
		<input class="inputBox marignInput" placeholder="请再次输入新密码"	@input="SureNewTel">
		<view class="sure comm-btn" @tap="sureBtn">确认</view>
	...
	export default {
		...
		methods: {
			...
			sureBtn(){
				var that = this;
				var re = /[A-Za-z]+/;
				var result = that.NewTelNum.search(re)
				if( that.codeNum == "" ){
					...
					return;
				}else if( that.SureTelNum == "" ){
					uni.showToast({icon:"none",title:"请确认新密码"})
					return;
				}else if( that.SureTelNum !== that.NewTelNum ){
					uni.showToast({icon:"none",title:"两次新密码输入不相同"})
					return;
				}else if( that.NewTelNum.length < 8 || that.NewTelNum.length > 16 ){
					...
					let param={
						...
						"code":that.codeNum,
						"pwd":that.NewTelNum,
						"re_pwd":that.SureTelNum
					};
					this._$InitRequest.request('user/change_pwd', param).then(res => {
						console.log( res );
						that.showDow = true
						if( res.data.code !== 200 ){
							...
							that.status.content = "您的修改密码已经修改成功 下次登陆请用新密码进行登录"
						}
					},error=>{})
				}
			},
			shadowSure(){uni.switchTab({url:"/pages/user/user"})}
