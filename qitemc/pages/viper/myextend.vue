<template>
	<view>
		<view class="extend-wraps" v-if="myTeam.length>0">
			<view class="extend-item" @click="toSonTeam(item.id,item.nickname)" v-for="(item,i) in myTeam" :key="i">
				...
				<view class="extend-text">
					...
					<view class="extend-num">
						团队成员  {{item.team_num}}人
					</view>
...
<script>
	...
	export default{
		...
		methods:{
			...
			toSonTeam(id,nickname){
				uni.navigateTo({
					url:'/pages/viper/sonTeam?otherId=' + id+'&nickname='+nickname
				})
			}
		}
	}
