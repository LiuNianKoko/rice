	...
	<view class="pay-wraps">
		...
		<view class="pay-main">
				...
				<view class="pay-text" v-for="(item,i) in payList" :key="i">
					...
					<view class="text-intr">
						...
						<view class="text-prices">
							<!-- <text class="symbol">￥</text>{{item.earnest_money[0]}} -->
							<view>
			...
			<!-- 	<view class="pay-card pay-common">
				...
				<view class="pay-tail1 pay-common">
					<view class="pay-left-common">阶段1：定金</view>
					<view class="pay-right-common"><text class="symbol">￥</text>{{totalMoney}}</view>
				</view>
				<view class="pay-tail2 pay-common">
					<view class="pay-left-common">阶段2：尾款</view>
					<view class="pay-right-common"><text class="symbol">￥</text>{{tailMoney}}</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			//#ifdef MP-WEIXIN
			let orderdata = wx.getStorageSync('presaleOrderdata')
			//#endif
			// let orderdata = this.$util.getStorage('presaleOrderdata');
		...
		methods:{
				...
				if(this.payType==1|| this.payType==2){
					...
					http('/api/order/pay_bank_order','get',data).then(res=>{
						...
						if(res.data.status == '200'){
							...
							if(!this.buycontent){
								url = this.domain_name + '/api/Order/pay_place_order/address_id/'+this.addressId+'/type/'+this.ordertype+'/payment_type/cloudFlash/order_type/3/token/'+token+'/spread_user_id/'+userid+'/presale_id/'+this.presaleId+'/unique_number/'+this.md5+'/data/'+data2;
							}else{
								url = this.domain_name + '/api/Order/pay_place_order/address_id/'+this.addressId+'/type/'+this.ordertype+'/payment_type/cloudFlash/order_type/3/token/'+token+'/spread_user_id/'+userid+'/presale_id/'+this.presaleId+'/buycontent/'+this.buycontent+'/unique_number/'+this.md5+'/data/'+data2;
							}
			...
			getPayList(){	
				...
				//#ifdef MP-WEIXIN
				let data = wx.getStorageSync('presaleOrderdata')
				data = {
					...data,
					address_id:this.addressId,
				}
				...
				http('/api/Presale/spikeOrder','post',data).then(res=>{
					console.log(res)
