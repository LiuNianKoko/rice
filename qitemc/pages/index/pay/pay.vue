	...
	<view class="pay-wraps">
		...
		<view class="pay-main">
			...
			<view class="pay-item">
				...
				<!-- <view class="pay-card pay-common" @tap="useCard" v-if="types==2">
				...
				<view class="pay-remarks pay-common">
					...
					<view class="pay-right-common">
						...
						<view class="pay-right-common pay-right">需要发票请联系客服</view>
					</view>
				</view>
		...
		<QSPopup ref="QSPopupChoose" type="fadeInUp">
			<view class="choosepay">
				...
				<!-- #ifndef APP-PLUS
					<view class="choose-item" v-for="(item,index) in chooseListwx" :key="index" @tap="chooses(item,index)">
						<view class="choose-item-l">
							<image :src="item.src"></image>
							{{item.name}}
						</view>
						<view class="choose-item-r">
							<image class="select-img" :src="item.checked ? selectImg : unSelectImg" @tap="chooses(item,index)"></image>
						</view>
					</view>
				 #endif  -->
		...
		<QSPopup ref="QSPopupYinhka">
			<view class="yinkka">
				<rich-text :nodes="yinhkaHTML"></rich-text>
			</view>
	...
	export default {
		...
		methods:{
				...
				if(this.payType==1|| this.payType==2){
					...
					http('/api/order/pay_bank_order','get',data).then(res=>{
						...
						if(res.data.status == '200'){
							...
							if(!this.buycontent){
								url = this.domain_name + '/api/Order/pay_place_order/address_id/'+this.addressId+'/type/'+this.types+'/payment_type/cloudFlash/order_type/'+this.tequan+'/token/'+token+'/spread_user_id/'+userid+'/coupon_id/'+this.couponId+'/unique_number/'+this.md5+'/data/'+data2;
							}else{
								url = this.domain_name + '/api/Order/pay_place_order/address_id/'+this.addressId+'/type/'+this.types+'/payment_type/cloudFlash/order_type/'+this.tequan+'/token/'+token+'/spread_user_id/'+userid+'/coupon_id/'+this.couponId+'/buycontent/'+this.buycontent+'/unique_number/'+this.md5+'/data/'+data2;
							}
			...
			getPayList(){	
				...
				let datas = {}
				//#ifndef MP-WEIXIN
				let data = this.$util.getStorage('orderdata')
				...
				// datas.address_id = this.addressId
				//#endif
				
				//#ifdef MP-WEIXIN
				let data = wx.getStorageSync('orderdata')
				datas = {
					...data,
					address_id : this.addressId
				}
				//#endif
				console.log(datas)
				http('/api/Order/create_order','post',datas).then(res=>{
					console.log(1);
					console.log(datas);
					console.log(res)
					if(res.data.status=='200'){
			...
			chooses(type, indexs){
				let that = this
				let cartList = that.chooseListwx.map((item, index) => {
					item.checked = false;
