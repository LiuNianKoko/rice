	...
	<view>
		<view class="sug-wraps">
			<view class="input-item">
				<input v-model="title" type="text" placeholder-class="color-gray"  placeholder="请输入意见反馈标题" maxlength="20"  class="input" />
			</view>
			<view class="input-textarea">
				<view class="input-area">
					<textarea v-model="content" placeholder-class="color-gray"  placeholder="请输入反馈内容"   class="inputs" />
				</view>
				...
				<view class="area-img" @click="didPressChooseImage" v-if="imgpathArr.length< 3">
					
					<image class="camera" src="/static/zhangh.png" ></image>
				</view>
			</view>
			<view class="input-item">
				<input v-model="userName" type="text" placeholder-class="color-gray" placeholder="请输入你的姓名" maxlength="10"  class="input" />
			</view>
			<view class="input-item">
				<input v-model="phone" type="text" placeholder-class="color-gray" placeholder="请输入你的联系方式" maxlength="11"  class="input" />
			</view>
			<view class="btn-wrap">
				<button class="btn" @click="Submission">立即提交</button>
			</view>
...
<script>
	...
	export default{
		...
		methods:{
			...
			Submission(){
				if(this.title==''){
					this.$util.showToast('请输入反馈标题')
					return
				}
				if(this.content==''){
					this.$util.showToast('请输入反馈内容')
					return
				}
				if(this.userName==''){
					this.$util.showToast('请输入您的姓名')
					return
				}
				if(this.phone==''){
					this.$util.showToast('请输入您的联系方式')
					return
				}
				let data = {
					token:uni.getStorageSync('token'),
