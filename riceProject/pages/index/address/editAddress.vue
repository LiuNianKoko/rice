	...
	<view class="shouhzid">
		<view class="header-wrap">
			<view class="index-header">
				...
				<text class="addNew" @click="del">删除</text>
			</view>
		</view>
		...
		<view class="ad-item friad">
			...
			<view class="item-content disflex border_bottom">
				...
				<view class="flex-1 ipt-fwben">
					<input type="number" v-model="phone" placeholder="手机号码或固定电话"></input>
				</view>
			</view>
		<!-- 	<view class="item-content disflex border_bottom">
				<view class="bti">
		...
		<view class="ad-item">
			<view class="item-content disflex border_bottom" @click="chooseCity">
				...
				<view class="flex-1 ipt-fwben">
					<input type="text" disabled="disabled" v-model="cityPickerValuea" placeholder="请选择省市区"></input>
			...
			<view class="item-content disflex border_bottom">
				...
				<view class="flex-1 ipt-fwben right">
					<input type="text"  v-model="xiangxidizhi" placeholder="街道门牌信息"></input>
				</view>
			</view>
		</view>
		<view class="row default-row box-sizing">
			<text class="tit left">设置默认地址</text>
			<switch class="swichbtn right margingRight20" color="#25928B" :checked="fault==1?true:false" @change="switchChange" />
		</view>
		...
		<!-- <view class="delete" @tap="show('QSPopup')">
			删除
		</view> -->
		<!-- 删除弹窗 -->
		<QSPopup ref="QSPopup">
			<view class="content-del">
				...
				<view class="del-btn">
					...
					<view class="del-btn-yes" @tap="delAddress">
						确认
					</view>
				</view>
			</view>
		...
		<QSPopup ref="QSPopupSuccess">
			<view class="content-del">
				...
				<view class="del-btn">
					<!-- <view class="del-btn-no" @tap="hides('QSPopupSuccess')">
						取消
					</view> -->
		...
		methods:{
			...
			getAddress(){
				...
				http('/api/user/getUserAddress','post',data).then(res=>{
					if(res.data.status == '200'){
						this.shouhren = res.data.data.address.username;
						this.phone = res.data.data.address.phone;
						this.cityPickerValuea = res.data.data.address.address;
						this.province = res.data.data.address.province;
						this.city = res.data.data.address.city;
						this.area = res.data.data.address.area;
						this.fault=res.data.data.address.default;
						this.xiangxidizhi =res.data.data.address.detail_address;
						console.log(this.cityPickerValuea)
					}
					console.log(res)
				})
			},
