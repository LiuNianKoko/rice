	...
	<view class="detail-wraps">
		...
		<view class="detail-wrap1" v-if="goodsStatus">
			<view class="detail-head" >
				...
				<!-- 预售信息 -->
					<view class="advance" v-if="goods.delivery_type==1">
						预售现在付款预计{{goods.delivery_time||''}}发货
					</view>
					<view class="advance" v-if="goods.delivery_type==2">
						尾款支付完成后{{goods.delivery_day||''}}天内发货
					</view>
					<view class="advance">
						尾款:{{goods.tail_starttime ||''}} - {{goods.tail_endtime||''}}
					</view>
					...
					<view class="detail-price">
						<view class="detail-vip-price">
							...
							{{goods.reveal_price || ''}}
							<image class="x2-img" src="/static/member-man4.png"></image>
						</view>
						<!-- <text class="detail-vip-price margingLeft20">预售价￥{{goods.reveal_price||''}}</text> -->
				...
				<view class="content-main" >
					<view class="content-text disFlex scale-1px-bottom paddingBottom20">
						...
						<view class="content-intr">
							<view class="content-price" v-if="type==1">
								<text class="content-cost-price">
									<text class="cost-symbol">￥</text>
									{{costprice}}
								</text>
							</view>
							<view class="content-price" v-if="type==2">
								<text class="content-vip-price">定金<text class="symbol">￥</text><text class="content-vip-prices">{{price}}</text></text>
								<text class="content-vip-price2 content-vip-price">尾款<text class="symbol">￥</text><text class="content-vip-prices">{{tailprice}}</text></text>
							</view>
	...
	export default {
		...
		methods:{
			...
			getInfo(){
				...
				http('/api/Presale/presaleInfo','post',data).then(res=>{
					if(res.data.status =='200'){
			...
			normGroup(item,index){
				...
				http('/api/Presale/ajaxCheckPresale','post',data).then(res=>{
					if(res.data.status == '200'){
			...
			toPay(){
					...
					if(this.type == 1){
						...
						if(this.buyNum ){
							//#ifndef MP-WEIXIN
							this.$util.setStorage('presaleOrderdata',{
								token:uni.getStorageSync('token'),
								type:1,
								goods_id:this.goodsId,
								attr_id:this.skuId,
