...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			//#ifdef MP-WEIXIN
			let orderdata = wx.getStorageSync('groupOrderdata')
			//#endif
			// let orderdata = this.$util.getStorage('groupOrderdata');
		...
		methods:{
				...
				if(this.payType==1|| this.payType==2){
					...
					http('/api/order/pay_bank_order','get',data).then(res=>{
						...
						if(res.data.status == '200'){
							...
							if(!this.buycontent){
								url = this.domain_name + '/api/Order/pay_place_order/address_id/'+this.addressId+'/type/'+this.ordertype+'/payment_type/cloudFlash/order_type/2/token/'+token+'/spread_user_id/'+userid+'/group_buy_id/'+this.group_buy_id+'/condition_number/'+this.condition_number+'/number/'+this.num+'/group_buy_records_id/'+this.group_buy_records_id+'/unique_number/'+this.md5+'/data/'+data2;
							}else{
								url = this.domain_name + '/api/Order/pay_place_order/address_id/'+this.addressId+'/type/'+this.ordertype+'/payment_type/cloudFlash/order_type/2/token/'+token+'/spread_user_id/'+userid+'/group_buy_id/'+this.group_buy_id+'/condition_number/'+this.condition_number+'/number/'+this.num+'/group_buy_records_id/'+this.group_buy_records_id+'/buycontent/'+this.buycontent+'/unique_number/'+this.md5+'/data/'+data2;
							}
			...
			getPayList(){
				...
				//#ifdef MP-WEIXIN
				let data = wx.getStorageSync('groupOrderdata')
				data = {
					...data,
					address_id:this.addressId,
				}
				...
				http('/api/GroupBuy/groupBuyOrder','post',data).then(res=>{
					console.log(res)
