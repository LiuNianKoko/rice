<template>
	<view class="setpay-wrap">
		<view class="itemes disflex border">
			...
			<input v-model="phone" disabled class="ipt flxe-1" type="text" value="" placeholder="请输入手机号" />
		</view> 
		<view class="itemes disflex border">
			<text class="label">验证码</text>
			<input v-model="code" class="ipt flxe-1" type="text" value="" placeholder="请输入短信验证码" />
			<view class="yanzma">
		...
		<view class="itemes disflex border">
			...
			<input v-model="password1" class="ipt flxe-1" type="password" value="" placeholder="请输入密码" />
		</view>
		<view class="itemes disflex border">
...
<script>
	...
	export default {
		...
		onLoad() {
			this.phone = this.$util.getStorage('phone')
		...
		methods: {
			...
			submit() {
				...
				http('/api/user/saveUserPaymentPassword','post',data).then(res=>{
					if(res.data.status == '200'){
						
						this.$util.showToast('设置成功')
						setTimeout(()=>{
							uni.switchTab({
								url:'/pages/mine/mine'
							})
